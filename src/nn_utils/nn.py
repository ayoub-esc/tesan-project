...
def dropout(x, keep_prob, is_train, noise_shape=None, seed=None, name=None):
    with tf.compat.v1.name_scope(name or "dropout"):
        assert is_train is not None
        if keep_prob < 1.0:
            d = tf.nn.dropout(x, rate=1 - (keep_prob), noise_shape=noise_shape, seed=seed)
            out = tf.cond(pred=is_train, true_fn=lambda: d, false_fn=lambda: x)
            return out
        return x
...
def softsel(target, logits, mask=None, scope=None):
    ...
    """
    with tf.compat.v1.name_scope(scope or "Softsel"):
        a = softmax(logits, mask=mask)
        target_rank = len(target.get_shape().as_list())
        out = tf.reduce_sum(input_tensor=tf.expand_dims(a, -1) * target, axis=target_rank - 2)
        return out
...
def softsel_with_dropout(target, logits, mask=None,
    ...
    :return: [..., d], dtype=float
    """
    with tf.compat.v1.name_scope(scope or "softsel_with_dropout"):
        a = softmax(logits, mask=mask)
        if keep_prob < 1.0:
            assert is_train is not None
            a = tf.cond(pred=is_train, true_fn=lambda: tf.nn.dropout(a, rate=1 - (keep_prob)), false_fn=lambda: a)
        target_rank = len(target.get_shape().as_list())
        out = tf.reduce_sum(input_tensor=tf.expand_dims(a, -1) * target, axis=target_rank - 2)
        return out
...
def _linear(xs, output_size, bias,bias_start=0., scope=None):
    with tf.compat.v1.variable_scope(scope or 'linear_layer'):
        ...
        W = tf.compat.v1.get_variable('W', shape=[input_size,output_size],dtype=tf.float32, use_resource=False)
        if bias:
...
def linear(args, output_size, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0, input_keep_prob=1.0,
    ...
    if input_keep_prob < 1.0:
        assert is_train is not None
        flat_args = [tf.cond(pred=is_train, true_fn=lambda: tf.nn.dropout(arg, rate=1 - (input_keep_prob)), false_fn=lambda: arg)# for dense layer [(-1, d)]
...
def linear_3d(tensor, hn, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0, input_keep_prob=1.0,
    ...
    with tf.compat.v1.variable_scope(scope or 'linear_3d'):
        ...
        weight_3d = tf.compat.v1.get_variable('weight_3d', [num_int, vec_int, hn], tf.float32, use_resource=False)

        if input_keep_prob < 1.0:
            assert is_train is not None
            tensor = tf.cond(pred=is_train, true_fn=lambda: tf.nn.dropout(tensor, rate=1 - (input_keep_prob)), false_fn=lambda: tensor)
        if bias:
            bias_3d = tf.compat.v1.get_variable('bias_3d', [num_int, 1, hn], tf.float32,
                                      tf.compat.v1.constant_initializer(bias_start), use_resource=False)
...
def conv1d(in_, filter_size, height, padding, is_train=None, keep_prob=1.0, scope=None):
    with tf.compat.v1.variable_scope(scope or "conv1d"):
        ...
        filter_ = tf.compat.v1.get_variable("filter", shape=[1, height, num_channels, filter_size], dtype='float', use_resource=False)
        bias = tf.compat.v1.get_variable("bias", shape=[filter_size], dtype='float', use_resource=False)
        strides = [1, 1, 1, 1]
        if is_train is not None and keep_prob < 1.0:
            in_ = dropout(in_, keep_prob, is_train)
        xxc = tf.nn.conv2d(input=in_, filters=filter_, strides=strides, padding=padding) + bias  # [N*M, JX, W/filter_stride, d] # (b,l,wl,d')
        out = tf.reduce_max(input_tensor=tf.nn.relu(xxc), axis=2)  # [-1, JX, d] # (b,l,d')
        return out
...
def sum_logits(args, mask=None, name=None):
    with tf.compat.v1.name_scope(name or "sum_logits"):
        if args is None or (isinstance(args, (tuple, list)) and not args):
            raise ValueError("`args` must be specified")
        if not isinstance(args, (tuple, list)):
            args = [args]
        rank = len(args[0].get_shape())
        logits = sum(tf.reduce_sum(input_tensor=arg, axis=rank - 1) for arg in args)
        if mask is not None:
            logits = exp_mask(logits, mask)
        return logits
# ------------------------------------------------------------
...
def feature_combination(org_tensor, new_features, wd=0., keep_prob=1., is_train=None, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or 'fea_comb'):
        bs, sl, vec = tf.shape(input=org_tensor)[0],tf.shape(input=org_tensor)[1],tf.shape(input=org_tensor)[2]
...
def pooling_with_mask(rep_tensor, rep_mask, method='max', scope=None):
    ...
    with tf.compat.v1.name_scope(scope or '%s_pooling' % method):

        if method == 'max':
            rep_tensor_masked = exp_mask_for_high_rank(rep_tensor, rep_mask)
            output = tf.reduce_max(input_tensor=rep_tensor_masked, axis=-2)
        elif method == 'mean':
            rep_tensor_masked = mask_for_high_rank(rep_tensor, rep_mask)  # [...,sl,hn]
            rep_sum = tf.reduce_sum(input_tensor=rep_tensor_masked, axis=-2)  #[..., hn]
            denominator = tf.reduce_sum(input_tensor=tf.cast(rep_mask, tf.int32), axis=-1, keepdims=True)  # [..., 1]
            denominator = tf.compat.v1.where(tf.equal(denominator, tf.zeros_like(denominator, tf.int32)),
...
def generate_embedding_mat(dict_size, emb_len, init_mat=None, extra_mat=None,
    ...
    with tf.compat.v1.variable_scope(scope or 'gene_emb_mat'):
        ...
        if init_mat is None:
            emb_mat_other = tf.compat.v1.get_variable('emb_mat',[dict_size - 2, emb_len], tf.float32, use_resource=False)
        else:
            emb_mat_other = tf.compat.v1.get_variable("emb_mat",[dict_size - 2, emb_len], tf.float32,
                                            initializer=tf.compat.v1.constant_initializer(init_mat[2:], dtype=tf.float32,
                                                                                verify_shape=True), use_resource=False)
        ...
        if extra_mat is not None:
            if extra_trainable:
                extra_mat_var = tf.compat.v1.get_variable("extra_emb_mat",extra_mat.shape, tf.float32,
                                                initializer=tf.compat.v1.constant_initializer(extra_mat,
                                                                                    dtype=tf.float32,
                                                                                    verify_shape=True), use_resource=False)
    ...
    with tf.compat.v1.variable_scope('token_and_char_emb'):
        if if_token_emb:
            with tf.compat.v1.variable_scope('token_emb'):
                ...
                c_token_emb = tf.nn.embedding_lookup(params=token_emb_mat, ids=context_token)  # bs,sl,tel

        if if_char_emb:
            with tf.compat.v1.variable_scope('char_emb'):
                char_emb_mat = generate_embedding_mat(cds, cel, scope='gene_char_emb_mat')
                c_char_lu_emb = tf.nn.embedding_lookup(params=char_emb_mat, ids=context_char)  # bs,sl,tl,cel
...
def generate_feature_emb_for_c_and_q(feature_dict_size, feature_emb_len,
    ...
    with tf.compat.v1.variable_scope(scope or '%s_feature_emb' % feature_name):
        ...
        c_feature_emb = tf.nn.embedding_lookup(params=emb_mat, ids=c_feature)
        if q_feature is not None:
            q_feature_emb = tf.nn.embedding_lookup(params=emb_mat, ids=q_feature)
        else:
            q_feature_emb = None
