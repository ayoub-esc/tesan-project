    ...
    with tf.compat.v1.variable_scope(scope or 'cnn_for_sentence_encoding'):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.compat.v1.variable_scope("conv-maxpool-%s" % filter_size):
                ...
                W = tf.compat.v1.get_variable('W', filter_shape, tf.float32, use_resource=False)
                b = tf.compat.v1.get_variable('b', [num_filters], tf.float32, use_resource=False)
                ...
                rep_tensor_expand_dp_pad = tf.pad(tensor=rep_tensor_expand_dp, paddings=padding)

                conv = tf.nn.conv2d(
                    input=rep_tensor_expand_dp_pad,
                    filters=W,
                    strides=[1, 1, 1, 1],
                    padding="VALID",
                    name="conv")
    ...
    with tf.compat.v1.variable_scope(scope or 'cnn_for_sentence_encoding'):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.compat.v1.variable_scope("conv-maxpool-%s" % filter_size):
                ...
                #     padding='VALID',
                #     name="pool")
                pooled = tf.reduce_max(input_tensor=h, axis=1, keepdims=True)  # bs, 1, 1, fn
    ...
    with tf.compat.v1.variable_scope(scope or 'cnn_for_sentence_encoding'):
        ...
        for layer_idx in range(layer_num):
            with tf.compat.v1.variable_scope("conv_maxpool_%s" % layer_idx):
                ...
                W = tf.compat.v1.get_variable('W', filter_shape, tf.float32, use_resource=False)
                b = tf.compat.v1.get_variable('b', [2 * ivec], tf.float32, use_resource=False)
                iter_rep_etd_pad = tf.pad(tensor=iter_rep_etd_dp, paddings=padding)
                conv = tf.nn.conv2d(
                    input=iter_rep_etd_pad,
                    filters=W,
                    strides=[1, 1, 1, 1],
                    padding="VALID",
                    name="conv")
