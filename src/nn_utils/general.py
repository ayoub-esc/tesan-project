...
def get_last_state(rnn_out_put, mask): # correct
    ...
    idxs = tf.reduce_sum(input_tensor=tf.cast(mask_flatten,tf.int32),axis=-1) - 1 # [X]
    indices = tf.stack([tf.range(tf.shape(input=idxs)[0]), idxs], axis=-1) #[X] => [X,2]
...
def expand_tile(tensor,pattern,tile_num = None, scope=None):
    with tf.compat.v1.name_scope(scope or 'expand_tile'):
...
# e.g. use:
initializer = slim.variance_scaling_initializer(factor=1.0, mode='FAN_IN')
