...
def traditional_attention(rep_tensor, rep_mask, scope=None,
    ...
    bs, sl, vec = tf.shape(input=rep_tensor)[0], tf.shape(input=rep_tensor)[1], tf.shape(input=rep_tensor)[2]
...
def multi_dimensional_attention(rep_tensor, rep_mask, keep_prob=1., is_train=None, wd=0., activation='relu'):
    ...
    with tf.compat.v1.variable_scope('multi_dimensional_attention'):
        ...
        attn_output = tf.reduce_sum(input_tensor=soft * rep_tensor, axis=1)  # bs, vec
...
def directional_attention_with_dense(
    ...
    with tf.compat.v1.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        if direction is None:
            direct_mask = tf.cast(tf.linalg.tensor_diag(- tf.ones([sl], tf.int32)) + 1, tf.bool)
        else:
        ...
        with tf.compat.v1.variable_scope('attention'):  # bs,sl,sl,vec
            f_bias = tf.compat.v1.get_variable('f_bias',[ivec], tf.float32, tf.compat.v1.constant_initializer(0.), use_resource=False)
            ...
            attn_result = tf.reduce_sum(input_tensor=attn_score * rep_map_tile, axis=2)  # bs,sl,vec

        with tf.compat.v1.variable_scope('output'):
            o_bias = tf.compat.v1.get_variable('o_bias',[ivec], tf.float32, tf.compat.v1.constant_initializer(0.), use_resource=False)
            # input gate
...
def normal_attention(rep_tensor, rep_mask, scope=None,
                          keep_prob=1., is_train=None, wd=0., activation='elu',
                          tensor_dict=None, name=None):
    batch_size, code_len, vec_size = tf.shape(input=rep_tensor)[0], tf.shape(input=rep_tensor)[1], tf.shape(input=rep_tensor)[2]
    ivec = rep_tensor.get_shape()[2]
...
def interval_with_dense(rep_tensor, rep_mask, interval_tensor, keep_prob=1.,
    ...
    with tf.compat.v1.variable_scope('temporal_attention'):
        ...
        time_rep_map = bn_dense_layer(interval_tensor, ivec, True, 0., 'bn_dense_map_time', activation,
        ...
        with tf.compat.v1.variable_scope('attention'):  # bs,sl,sl,vec
            ...
            if is_scale:
                ...
                fact_bias = tf.compat.v1.get_variable('fact_bias', [ivec], tf.float32, tf.compat.v1.constant_initializer(0.), use_resource=False)
...
def temporal_interval_sa_with_dense(rep_tensor, rep_mask, interval_tensor, keep_prob=1.,
                                     is_train=None, wd=0., activation='relu', hn=None, is_scale=True):
...
def time_aware_attention(train_inputs, embed, mask, embedding_size, k):
    with tf.compat.v1.variable_scope('time_aware_attention'):
        ...
        # get abs of distance
        train_interval = tf.abs(train_inputs[:, :, 1])

        # distance function is log(dist+1)
        dist_fun = tf.math.log(tf.cast(train_interval, dtype=tf.float32) + 1.0)

        # reshape the dist_fun
        dist_fun = tf.reshape(dist_fun, [tf.shape(input=dist_fun)[0], tf.shape(input=dist_fun)[1], 1])

        # the attribution logits
        attn_logits = tf.multiply(attn_scalars, dist_fun)

        # the attribution logits sum
        attn_logits_sum = tf.reduce_sum(input_tensor=attn_logits, axis=-1, keepdims=True)
        ...
        reduced_embed = tf.reduce_sum(input_tensor=attn_embed_weighted, axis=1)
        # obtain two scalars
        scalar1 = tf.math.log(tf.cast(tf.shape(input=embed)[1], dtype=tf.float32) + 1.0)
        scalar2 = tf.reduce_sum(input_tensor=tf.pow(attn_softmax, 2), axis=1)
        # the scalared embed
...
def visit_temporal_date_sa_with_dense(rep_tensor, date_tensor,
    ...
    batch_size, sw_len, vec_size = tf.shape(input=rep_tensor)[0], tf.shape(input=rep_tensor)[1], tf.shape(input=rep_tensor)[2]
    ivec = rep_tensor.get_shape().as_list()[2]
    ivec = hn or ivec
    with tf.compat.v1.variable_scope('temporal_attention'):
        # mask generation
        attn_mask = tf.cast(tf.linalg.tensor_diag(- tf.ones([sw_len], tf.int32)) + 1, tf.bool)  # batch_size, code_len, code_len
...
def first_level_sa(rep_tensor, rep_mask, keep_prob=1., is_train=None, wd=0., activation='relu'):
    ...
    with tf.compat.v1.variable_scope('first_level_sa'):
        ...
        attn_output = tf.reduce_sum(input_tensor=soft * rep_tensor, axis=2)  # bs, sk, vec
