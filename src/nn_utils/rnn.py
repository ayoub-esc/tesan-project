...
def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
    ...
    flat_outputs, final_state = tf.compat.v1.nn.dynamic_rnn(cell, flat_inputs, sequence_length=flat_len,
...
def bw_dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
    ...
    flat_inputs = tf.reverse(flat_inputs, [1]) if sequence_length is None \
        else tf.reverse_sequence(input=flat_inputs, seq_lengths=sequence_length, seq_axis=1)
    flat_outputs, final_state = tf.compat.v1.nn.dynamic_rnn(cell, flat_inputs, sequence_length=flat_len,
                                                  initial_state=initial_state, dtype=dtype,
                                                  parallel_iterations=parallel_iterations,
                                                  swap_memory=swap_memory,
    ...
    flat_outputs = tf.reverse(flat_outputs, [1]) if sequence_length is None \
        else tf.reverse_sequence(input=flat_outputs, seq_lengths=sequence_length, seq_axis=1)
...
def bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
    ...
    (flat_fw_outputs, flat_bw_outputs), final_state = \
        tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, flat_inputs, sequence_length=flat_len,
...
# ---------------- RNN Cell ----------------
class SwitchableDropoutWrapper(tf.compat.v1.nn.rnn_cell.DropoutWrapper):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        outputs = tf.cond(pred=self.is_train, true_fn=lambda: outputs_do, false_fn=lambda: outputs)
        if isinstance(state, tf.compat.v1.nn.rnn_cell.LSTMStateTuple):
            new_state = state.__class__(*[tf.cond(pred=self.is_train, true_fn=lambda: new_state_do_i, false_fn=lambda: new_state_i)
                                          for new_state_do_i, new_state_i in zip(new_state_do, new_state)])
        elif isinstance(state, tuple):
            new_state = state.__class__([tf.cond(pred=self.is_train, true_fn=lambda: new_state_do_i, false_fn=lambda: new_state_i)
                                         for new_state_do_i, new_state_i in zip(new_state_do, new_state)])
        else:
            new_state = tf.cond(pred=self.is_train, true_fn=lambda: new_state_do, false_fn=lambda: new_state)
        return outputs, new_state


class NormalSRUCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope or "SRU_cell"):
            b_f = tf.compat.v1.get_variable('b_f', [self._num_units], dtype=tf.float32,
                                  initializer=tf.compat.v1.constant_initializer(0), use_resource=False)
    ...
    with tf.compat.v1.variable_scope(scope or 'bi_sru_recurrent_network'):
        ...
        with tf.compat.v1.variable_scope('forward'):
            ...
            fw_output, _ = dynamic_rnn(
                fw_SRUCell, U_fw, tf.reduce_sum(input_tensor=tf.cast(rep_mask, tf.int32), axis=-1),
        ...
        with tf.compat.v1.variable_scope('backward'):
            ...
            bw_output, _ = bw_dynamic_rnn(
                bw_SRUCell, U_bw, tf.reduce_sum(input_tensor=tf.cast(rep_mask, tf.int32), axis=-1),
...
class SRUCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self, num_units, activation=None, reuse=None):
...
def contextual_bi_rnn(tensor_rep, mask_rep, hn, cell_type, only_final=False,
    ...
    with tf.compat.v1.variable_scope(scope or 'contextual_bi_rnn'): # correct
        ...
        if cell_type == 'sru':
            ...
            if cell_type == 'gru':
                cell_fw = tf.compat.v1.nn.rnn_cell.GRUCell(hn, reuse=reuse)
                cell_bw = tf.compat.v1.nn.rnn_cell.GRUCell(hn, reuse=reuse)
            elif cell_type == 'lstm':
                cell_fw = tf.compat.v1.nn.rnn_cell.LSTMCell(hn, reuse=reuse)
                cell_bw = tf.compat.v1.nn.rnn_cell.LSTMCell(hn, reuse=reuse)
            elif cell_type == 'basic_lstm':
                cell_fw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hn, reuse=reuse)
                cell_bw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hn, reuse=reuse)
            elif cell_type == 'basic_rnn':
                cell_fw = tf.compat.v1.nn.rnn_cell.BasicRNNCell(hn, reuse=reuse)
                cell_bw = tf.compat.v1.nn.rnn_cell.BasicRNNCell(hn, reuse=reuse)
            ...
            tensor_len = tf.reduce_sum(input_tensor=tf.cast(mask_rep, tf.int32), axis=-1)  # [bs]
