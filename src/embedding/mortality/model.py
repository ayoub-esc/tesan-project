...
class MortalityModel(ModelTemplate):
    ...
    def __init__(self,scope, dataset):
        ...
        # ------------ other ---------
        self.batch_size = tf.shape(input=self.inputs)[0]
    ...
    def build_loss_optimizer(self):
        with tf.compat.v1.name_scope('loss_optimization'):
            ...
            optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.0001).minimize(loss)
    ...
    def last_relevant(self):
        batch_size = tf.shape(input=self.outputs)[0]
        max_length = tf.shape(input=self.outputs)[1]
    ...
    def build_network(self):
        ...
        with tf.compat.v1.name_scope('visit_embedding'):
            # bs, max_visits, max_len_visit, embed_size
            inputs_masked = mask_for_high_rank(inputs_embed, self.inputs_mask)
            inputs_reduced = tf.reduce_mean(input_tensor=inputs_masked, axis=2)  # batch_size, max_visits, embed_size

        with tf.compat.v1.name_scope('visit_masking'):
            visit_mask = tf.reduce_sum(input_tensor=tf.cast(self.inputs_mask, tf.int32), axis=-1)  # [bs,max_visits]
            visit_mask = tf.cast(visit_mask, tf.bool)
            tensor_len = tf.reduce_sum(input_tensor=tf.cast(visit_mask, tf.int32), axis=-1)  # [bs]

        with tf.compat.v1.name_scope('RNN_computaion'):
            reuse = None if not tf.compat.v1.get_variable_scope().reuse else True
            if cfg.cell_type == 'gru':
                cell = tf.compat.v1.nn.rnn_cell.GRUCell(cfg.hn, reuse=reuse)
            elif cfg.cell_type == 'lstm':
                cell = tf.compat.v1.nn.rnn_cell.LSTMCell(cfg.hn, reuse=reuse)
            elif cfg.cell_type == 'basic_lstm':
                cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(cfg.hn, reuse=reuse)
            elif cfg.cell_type == 'basic_rnn':
                cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(cfg.hn, reuse=reuse)
